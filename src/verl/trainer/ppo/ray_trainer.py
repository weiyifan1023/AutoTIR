...
def compute_data_metrics(batch, use_critic=True):
    ...
    sequence_format_reward = batch.batch['token_level_format_scores'].sum(-1)
    ...
    metrics = {
        ...
        # format
        'critic/format/mean':
            torch.mean(sequence_format_reward).detach().item(),
        # adv
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            reward_tensor, format_tensor = self.val_reward_fn(test_batch, os.path.join(self.config.trainer.rollout_save_path, f'val_{self.global_steps}.jsonl'))
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        reward_tensor, format_tensor = self.reward_fn(batch, os.path.join(self.config.trainer.rollout_save_path, f'train_{self.global_steps}.jsonl'))
                        batch.batch['token_level_scores'] = reward_tensor
                        batch.batch['token_level_format_scores'] = format_tensor
