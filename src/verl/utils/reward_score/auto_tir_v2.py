...
from .if_eval.instruction_following_algorithm_environment import IF_FUNCTIONS_MAP
def validate_format(text: str, use_tool: bool = None) -> tuple[bool, str]:
    ...
    def check_tool_format(text: str) -> tuple[bool, str]:
        current_pos = 0
        has_tool_tags = False
        while True:
            search_pos = text.find('<search>', current_pos)
            code_pos = text.find('<code>', current_pos)
            if search_pos == -1 and code_pos == -1:
                # return False, "No <search> or <code> tags found"
                break
            has_tool_tags = True
            ...
            if search_pos != -1:
                tool_pos = search_pos
                tool_end_pos = text.find('</search>', search_pos)
                result_pos = text.find('<result>', search_pos)
            else:
                # code_pos != -1
                tool_pos = code_pos
                tool_end_pos = text.find('</code>', code_pos)
                result_pos = text.find('<result>', code_pos)

            result_end_pos = text.find('</result>', result_pos)

            if -1 in (tool_end_pos, result_pos, result_end_pos):
                return False, "tool/result tags are incomplete"

            if not (tool_pos < tool_end_pos < result_pos < result_end_pos):
                return False, "tool/result tags are nested in the wrong order"

            current_pos = result_end_pos

        if not has_tool_tags:
            return False, "No tool invocation tags (<search> or <code>) found."
        return True, "tool/result tags are in the correct order"
    ...
    # If either format is valid, then it's good for this part
    if use_tool is None:
        tool_flag, tool_reason = check_tool_format(text)
        nontool_flag, nontool_reason = check_nontool_format(text)

        # Explicitly check for tool tags for tool_flag
        if nontool_flag:
            pass
        elif tool_flag:
            pass
        else:
            # Give priority to displaying errors in non-tool formats.
            if not nontool_flag:
                return False, nontool_reason
            else:
                return False, tool_reason

    # check if response text contains tool invocation format
    elif use_tool:
        flag, reason = check_tool_format(text)
        if not flag:
            return False, reason
    else:  # not use_tool
        flag, reason = check_nontool_format(text)
        if not flag:
            return False, reason
...
def get_ifeval_score(answer: str, ground_truth: Union[str, List[str]]) -> Tuple[float, str]:
    """
...
def compute_score(tokenizer, solution_str, ground_truth, use_tool=True) -> float:
    ...
    if "func_name" in ground_truth[0]:  # for RLVR-IFeval
