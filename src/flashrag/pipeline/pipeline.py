...
class SequentialPipeline(BasicPipeline):
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        # retrieval_results = [""] * len(input_query)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm(as_completed(future_to_prompt), total=len(future_to_prompt), desc="Inference: "):
                i = future_to_prompt[future]
                pred_answer_list[i] = future.result()[0] if future.result() else ""  # future.result()[0]
        dataset.update_output("pred", pred_answer_list)
        dataset = self.evaluate(dataset, do_eval=do_eval, pred_process_fun=pred_process_fun)

        return dataset

    def medrun(self, dataset, do_eval=True, pred_process_fun=None):
        input_query = dataset.input
        retrieval_results = self.retriever.batch_search(input_query)
        dataset.update_output("retrieval_result", retrieval_results)
        ...
        if self.refiner:
            ...
            if "llmlingua" in self.refiner.name and input_prompt_flag:
                ...
                input_prompts = [
                    self.prompt_template.get_string(question=q, retrieval_result=r)
                    for q, r in zip(dataset.input, dataset.retrieval_result)
                ]
                dataset.update_output("prompt", input_prompts)
                input_prompts = self.refiner.batch_run(dataset)
            else:
                ...
                input_prompts = [
                    self.prompt_template.get_string(question=q, formatted_reference=r)
                    for q, r in zip(dataset.input, refine_results)
                ]
