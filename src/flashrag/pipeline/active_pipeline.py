...
class ReSearchPipeline(BasicPipeline):
    ...
    def run_item(self, item):
        ...
        while remain_length > 0:
            response = self.generator.generate(input_list=[query], return_raw_output=True, stop=['</search>'],
                                               max_new_tokens=remain_length)
            if response is None:
                print(f"[Warning] Generator returned None for query: {item.question!r}")
                item.update_output("final_response", "generation failed: generator returned None")
...
class IKEAPipeline(BasicPipeline):
    """
    https://github.com/hzy312/knowledge-r1/blob/main/infer.py
    """
    ...
    def __init__(self, config, retriever=None, generator=None, apply_chat=False):
        ...
        self.apply_chat = apply_chat
        self.tokenizer = AutoTokenizer.from_pretrained(config["generator_model_path"])
        self.prompt_template = prompt_template_dict['ikea_template_sys']  # instruction model
    def extract_search_content(self, text: str) -> str:
        try:
            start_tag = '<search>'
            end_tag = '</search>'
...
class SearchR1Pipeline(BasicPipeline):
    ...
    def __init__(self, config, retriever=None, generator=None, apply_chat=False):
        ...
        self.apply_chat = apply_chat
        self.tokenizer = AutoTokenizer.from_pretrained(config["generator_model_path"])
        self.prompt_template = prompt_template_dict['search_r1_template_sys']
    def extract_search_content(self, text: str) -> str:
        try:
            start_tag = '<search>'
            end_tag = '</search>'
    ...
    def run_item(self, item):
        if self.apply_chat:
            query = self.tokenizer.apply_chat_template([
                {'role': 'system', 'content': self.prompt_template},  # system instruction
                {'role': 'user', 'content': item.question}
            ], tokenize=False, add_generation_prompt=True)
        else:
            query = self.prompt_template.format(prompt=item.question)
        ...
        while remain_length > 0:
            ...
            if stop_reason == 'stop' and isinstance(stop_matched, str) and '</search>' in stop_matched:
                ...
                query += f"{output_str} <information>\n{retrieval_text}\n</information>"
            elif stop_reason == 'stop' and (stop_matched == 151643 or stop_matched == 151645):
                output_str = response['text']
                query += f"{output_str}"
                break
...
class ToRLPipeline(BasicPipeline):
    def __init__(self, config, retriever=None, generator=None, apply_chat=False):
        super().__init__(config)
        if retriever is None:
            retriever = get_retriever(config)
        ...
        self.apply_chat = apply_chat
        if apply_chat:
            self.prompt_template = prompt_template_dict['torl_template_sys']
        else:
            self.prompt_template = prompt_template_dict['torl_template']  # re_tool_template
    ...
    def code_interpreter_call(self, tool_input, timeout=10):
        ...
        # 统一封装请求格式
        tool_post_input = {'code': tool_input, 'language': 'python'}

        result = send_request(tool_post_input)
        return result

    def extract_boxed(self, text: str) -> str:
        """
        Find the answer within the mathematical environment that is in the form of \boxed{...},
        and return the content within {...}.
        """
    ...
    def run_item(self, item):
        ...
        while remain_length > 0:
            ...
            response = self.generator.generate(input_list=[query], return_raw_output=True,
                                               stop=["```output"], max_new_tokens=remain_length)
            ...
            if stop_reason == 'stop' and isinstance(stop_matched, str) and "```output" in stop_matched:
                output_str = response['text']  # "```output"
                code_content = self.extract_code_content(output_str)
                if code_content != '':
                    code_results = self.code_interpreter_call(code_content)
                else:
                    code_results = 'nothing to run python code'

                query += f"{output_str} ```output\n{code_results}\n```"
        ...
        if over_length_flag:
            ...
            boxed_content = self.extract_boxed(final_response)
...
class PRIMEPipeline(BasicPipeline):
    """
    Eurus-2-7B-PRIME and Prompt: download from https://huggingface.co/PRIME-RL/Eurus-2-7B-PRIME
    """
    def __init__(self, config, retriever=None, generator=None, apply_chat=False):
        super().__init__(config)
        if retriever is None:
    ...
    def run_item(self, item):
        query = self.tokenizer.apply_chat_template([
            # {'role': 'system', 'content': self.prompt_template},
            {'role': 'user', 'content': item.question + self.prompt_template}
        ], tokenize=False, add_generation_prompt=True)
        ...
        while remain_length > 0:
            # llm generate response, stop at eos token or ```output
            response = self.generator.generate(input_list=[query], return_raw_output=True,
                                               trust_remote_code=True, max_new_tokens=remain_length)
            ...
            if stop_reason == 'stop' and (stop_matched == self.tokenizer.pad_token_id or stop_matched == self.tokenizer.eos_token_id):
                output_str = response['text']
                query += f"{output_str}"
                break
            elif stop_reason == 'length':
...
class SimpleRLPipeline(BasicPipeline):
    """
    SimpleRL-Zero: download from https://huggingface.co/hkust-nlp/Qwen-2.5-Math-7B-SimpleRL-Zero
    """
    def __init__(self, config, retriever=None, generator=None, apply_chat=False):
        super().__init__(config)
        if retriever is None:
        ...
        self.apply_chat = apply_chat
        self.tokenizer = AutoTokenizer.from_pretrained(config["generator_model_path"])
        # Prompt from Figure 10 in the Paper https://arxiv.org/pdf/2503.18892
    ...
    def run_item(self, item):
        ...
        while remain_length > 0:
            # llm generate response, stop at eos token or ```output
            response = self.generator.generate(input_list=[query], return_raw_output=True, max_new_tokens=remain_length)
...
class AutoTIRPipeline(BasicPipeline):
    def __init__(self, config, retriever=None, generator=None, apply_chat=False):
        super().__init__(config)
        if retriever is None:
            retriever = get_retriever(config)
        ...
        self.apply_chat = apply_chat
        if apply_chat:
            self.prompt_template = prompt_template_dict['autotir_template_sys']
        else:
            self.prompt_template = prompt_template_dict['re_search_template']  # re_tool_template
    ...
    def run_item(self, item):
        ...
        while remain_length > 0:
            response = self.generator.generate(input_list=[query], return_raw_output=True,
                                               stop=['</search>', '</code>'], max_new_tokens=remain_length)
            ...
            elif stop_reason == 'stop' and isinstance(stop_matched, str) and '</code>' in stop_matched:
                output_str = response['text'] + '</code>'
                code_content = self.extract_code_content(output_str)
                if code_content != '':
                    # code_results = self.code_interpreter_batch_call([code_content])
                    code_results = self.code_interpreter_call(code_content)
                else:
                    code_results = 'nothing to run code'
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        for iter_idx in range(self.iter_num):
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_prompt = {executor.submit(self.generator.generate, prompt): i for i, prompt in
                                    enumerate(input_prompts)}
                past_generation_result = [None] * len(input_prompts)
                for future in tqdm(as_completed(future_to_prompt), total=len(future_to_prompt), desc="Inference: "):
                    i = future_to_prompt[future]
                    past_generation_result[i] = future.result()[0] if future.result() else ""  # future.result()[0]
                    # past_generation_result[i] = future.result()[0]
...
class SelfRAGPipeline(BasicPipeline):
    ...
    def judge_retrieve(self, input_prompts):
        ...
        if self.mode == "always_retrieve":
            ...
            preds = self.generator.generate(input_prompts, return_raw_output=True, logprobs=20, max_tokens=1,
...
class FLAREPipeline(BasicPipeline):
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        max_workers = 50
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(self.run_item, item): item for item in dataset}
            for future in tqdm(as_completed(futures), total=len(futures), desc="Inference: "):
        ...
        while iter_num < self.max_iter:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_prompt = {executor.submit(self.generator.generate, prompt, stop=['.', '\n']): i for i, prompt
                                    in enumerate(input_prompts)}
                new_thoughts_batch = [None] * len(input_prompts)
                for future in tqdm(as_completed(future_to_prompt), total=len(future_to_prompt), desc="Inference: "):
                    i = future_to_prompt[future]
                    if future.result() is None:  # 
                        new_thoughts_batch[i] = "Error"
                    else:
                        new_thoughts_batch[i] = future.result()[0]
            ...
            if active_item_ids:
                ...
                new_retrieval_results, new_scoress = self.retriever.batch_search(new_thoughts_for_retrieval,
                                                                                 return_score=True)
