...
from verl.utils.reward_score.if_eval.instruction_following_algorithm_environment import IF_FUNCTIONS_MAP
# from instruction_following_algorithm_environment import IF_FUNCTIONS_MAP
...
class IFEvalScore(BaseMetric):
    """Instruction-Following Evaluation Score using IFEval Verifier"""
    ...
    def compute_if_score(self, prediction: str, ground_truth_json: str) -> float:
        try:
        ...
        args = {k: v for k, v in gt.items() if k != "func_name"}

        try:
            raw = verifier_func(prediction, **args)
    ...
    def calculate_metric(self, data):
        """Calculate IFEval score for all samples

        Assumes:
            - data.pred: list of model predictions (strings)
            - data.golden_answers: list of JSON strings (ground truth verifier spec)
        """
        pred_list = data.pred
        ground_truth_list = data.golden_answers  # each is a JSON string like {"func_name": ..., ...}

        metric_score_list = [
            self.compute_if_score(pred, gt)
            for pred, gt in zip(pred_list, ground_truth_list)
        ]
        if_score = sum(metric_score_list) / len(metric_score_list)
        return {"ifeval": if_score}, metric_score_list
