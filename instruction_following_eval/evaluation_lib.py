...
from src.verl.utils.reward_score.if_eval import instructions_registry
...
def test_instruction_following_loose(
    ...
    # response = prompt_to_response[inp.prompt]  # 数据问题，有的prompt key不存在
    response = prompt_to_response.get(inp.prompt, "")
    r = response.split("\n")
...
def mutorl_read_prompt_to_response_dict(input_json_filename):
    """Creates dictionary matching prompt and response."""
    return_dict = {}
    with open(input_json_filename, "r") as f:
        ...
        for sample in data:
            ...
            return_dict[prompt] = sample.get("output", {}).get("final_response")
